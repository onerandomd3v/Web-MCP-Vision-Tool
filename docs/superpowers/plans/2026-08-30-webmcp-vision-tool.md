...
- Test: existing project checks through the client/server build scripts and Vitest.
...
- [x] Verify the Codespace has Node 24.14.1, Docker, and reachable PostgreSQL.
- [x] Run `npm run test` if present, otherwise `npx vitest run` only for test execution (GitHub Actions CI passed `npm test`).
- [x] Start the Vite client and Express API; verify the forwarded client and server ports are reachable.
...
- [x] Run the focused tests and client/server builds, then commit `feat: add targeted product image tool`.
...
- [x] Run all tests and client/server builds (covered by the passing GitHub Actions CI run).
...
**Files:**
- Configure Railway directly for the Express server; no Fly manifests are part of this deployment.
- Create: `docs/deployment.md` covering client hosting, Railway API hosting, Neon, environment variables, and migrations.
